...
final class NavigationUrlClassifierTests: XCTestCase {
    ...
    /** Tests A: server error query is classified for error dialog path. */
    func testServerErrorUrlIsDetected() {
        XCTAssertTrue(NavigationUrlLogic.isServerErrorUrl("https://server/Login.aspx?Error=-6"))
        XCTAssertFalse(NavigationUrlLogic.isServerErrorUrl("https://server/Default.aspx"))
    }

    /** Tests A: special routes hide WebView (barcode, login, about:blank). */
    func testShouldHideWebViewForSpecialRoutes() {
        XCTAssertTrue(NavigationUrlLogic.shouldHideWebView("barcodescanner://host/path"))
        XCTAssertTrue(NavigationUrlLogic.shouldHideWebView("https://server/login.aspx"))
        XCTAssertTrue(NavigationUrlLogic.shouldHideWebView("about:blank"))
        XCTAssertFalse(NavigationUrlLogic.shouldHideWebView("https://server/Default.aspx"))
    }
}
...
enum NavigationUrlLogic {
    ...
    static func shouldHideWebView(_ url: String) -> Bool {
        let lower = url.lowercased()
        return lower.hasPrefix(AppSettings.BARCODESCANNER)
            || lower.contains(AppSettings.LOGIN)
            || lower.contains(AppSettings.ABOUT_BLANK)
    }
}
