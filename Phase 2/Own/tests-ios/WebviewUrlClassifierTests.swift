...
final class WebviewUrlClassifierTests: XCTestCase {
    ...
    func testClassifyUrlOutcomes() {
        ...
        XCTAssertEqual(WebviewUrlLogic.classify("https://server/Login.aspx?Error=-6"), .error)
        XCTAssertEqual(WebviewUrlLogic.classify("about:blank"), .hidden)
    }

    /** Tests A: cross-platform route constants for WebView. */
...
enum WebviewUrlLogic {
    enum Outcome { case scanner, login, error, hidden, normal }
    ...
    static func classify(_ url: String) -> Outcome {
        ...
        if lower.contains(AppSettings.ERROR) { return .error }
        if lower.contains(AppSettings.LOGIN) { return .login }
        if lower.contains(AppSettings.ABOUT_BLANK) { return .hidden }
        return .normal
    }
}
