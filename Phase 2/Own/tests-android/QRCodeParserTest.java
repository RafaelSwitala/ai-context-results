...
public class QRCodeParserTest {
    ...
    /** LT-008 (Tests A): query-only payload normalizes then parses as valid MB QR. */
    @Test
    public void parse_normalizesQueryOnlyPayload() {
        String queryOnly = "p=MB&v=1&server=test.example.com&mandant=108&https=1&token=&pin=1234";
        String normalized = queryOnly.contains("?") ? queryOnly : "http://localhost?" + queryOnly;

        assertTrue(QRCodeParser.parse(normalized).isValid());
    }
}
